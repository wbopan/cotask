...
- [ ] Skill 规范补充：backlog 用法与 phase 创建准则 #skill-backlog-phase-guidance
    在 task-management skill 中补充实践指南：(1) backlog `[-]` 适用于"好主意但时机未到"的任务，不是垃圾桶；(2) Phase 代表宏大的里程碑目标，不应为零碎功能随意创建——如果几个任务不构成一个有凝聚力的阶段性目标，就放进现有 phase。
