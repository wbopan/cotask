...
- [x] Skill 规范补充：backlog 用法与 phase 创建准则 #skill-backlog-phase-guidance
    在 task-management skill 中补充实践指南：(1) backlog `[-]` 适用于"好主意但时机未到"的任务，不是垃圾桶；首先确认一下，现在的 SPEAR 里面这些东西都没怎么描述
    CM: 重写 SKILL.md backlog 段落：明确 backlog 是已识别、值得做但未排入当前焦点的工作项，预期未来会被拉入执行；真正废弃的任务应删除。Phase 创建准则因即将移除 phase 概念而跳过。
    AC: Skill 文档包含 backlog 使用场景说明和 phase 创建门槛准则；AI 遵循规范时不会为单个小任务创建新 phase。
- [/] 移除 Phase 概念，替换为可选的 Section #remove-phase-concept
